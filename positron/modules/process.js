...
// Per <https://nodejs.org/api/process.html#process_process_platform>, valid
// values for this property are darwin, freebsd, linux, sunos and win32; so we
// convert winnt to win32.  We also lowercase the value, but otherwise we don't
// modify it, so it *might* contain a value outside the Node set, per OS_TARGET
// <https://developer.mozilla.org/en-US/docs/Mozilla/Developer_guide/Build_Instructions/OS_TARGET>.
process.platform = runtime.OS.toLowerCase().replace(/^winnt$/, win32);
